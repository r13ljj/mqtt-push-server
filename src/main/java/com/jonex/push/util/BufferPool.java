...
public class BufferPool{
	...
	static{
		// 设置缓冲区池上限大小
		Integer maxSize = MqttProperties.getPropertyToInt(MAX_BUFFER_POOL_SIZE);// 获取配置中的线程优先级
		if(maxSize != null){
			maxBufferPoolSize = maxSize;
		}
		
		// 设置缓冲区池下限大小
		Integer minSize = MqttProperties.getPropertyToInt(MIN_BUFFER_POOL_SIZE);// 获取配置中的线程优先级
		if(minSize != null){
			minBufferPoolSize = minSize;
		}
		
		// 设置响应缓冲区大小
		Integer bufferSize = MqttProperties.getPropertyToInt(WRITE_BUFFER);
		if(bufferSize != null){
